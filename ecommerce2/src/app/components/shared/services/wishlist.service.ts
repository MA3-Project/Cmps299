...
export class WishlistService {
  ...
  public addToWishlist(product: Product): Product | boolean {
    ...
    if (this.hasProduct(product)) {
      ...
      this.snackBar.open('Error: The product  ' + product.name + ' is already added to your wishlist.', '×', { panelClass: [status]/*'error'*/, verticalPosition: 'top', duration: 3000 });
    } else {
      products.push(product);
    
      message = 'The product ' + product.name + ' has been added to your wishlist.';
      status = 'success';
